...
#function that is called any time someone tries to upload pdf; receive the upload
    res, status_code = files.upload(file_path)
    if status_code >= 400:
        return res, status_code

    #create a record inside the database for the file
    pdf = Pdf.create(id=file_id, name=file_name, user_id=g.user.id)

    # Defer this to be processed by the worker; adding .delay will send it to the worker rather than immediately process
    # this makes it so that after upload, the progress bar doesn't just 'hang' and leave you waiting.
